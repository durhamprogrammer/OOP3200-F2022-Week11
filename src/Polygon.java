public class Polygon extends Shape
{
    ...
    public Polygon(Vector2[] vertexArray) throws Exception
    {
        ...
        {
            throw new Exception("ERROR: Polygon is constructed with less than 3 vertices");
        }
    }
    ...
    private void checkIfAnyVerticesAreEqual(Vector2[] vertexArray) throws Exception
    {
        for (int i = 0; i < vertexArray.length - 1; i++)
        {
            for (int j = i + 1; j < vertexArray.length; j++) {
                ...
                {
                    throw new Exception("ERROR: Polygon constructed with at least one vertex that is the same as another one");
                }
            }
        }
    }
